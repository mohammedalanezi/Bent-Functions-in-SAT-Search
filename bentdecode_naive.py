...
class BentFunctionDecoder:
	...
	def __init__(self, encoder_instance, sat_output: str = None):
		...
		# Variable mappings from encoder
		...
		print("maybe build this off of decoder instead")
	...
	def anf_to_string(self, anf_coeffs: Optional[Dict] = None) -> str:
		...
		sorted_subsets = sorted(anf_coeffs.keys(), key=lambda s: (len(s), s))
...
if __name__ == "__main__":
	# Example: How to use after encoding and solving
	
	input_variables = 8
	max_degree = 2

	# First, encode the problem
	encoder = encode_and_save_bent_function(input_variables, max_degree, output_file=f"test_bent_{input_variables}_{max_degree}.cnf") 
	...
	if decoding.get_satisfiability():
		...
		#print(decoder.decode_truth_table())
	else:
