/**
 * bentdecode_truncated.cpp
 * By Mohammed Al-Anezi
 *
 * Bent Function SAT Encoder + Decoder (truncated / optimised variant).
 *
 * Key optimisations over the naïve Python encoder:
 *
 *   1. Phase variable elimination. s_{x,ω} = F_x ⊕ ⟨ω,x⟩ is always ¬F_x or F_x depending on the
 *      parity of ⟨ω,x⟩, so we pass the correct truth-table literal directly to the cardinality encoder (Step 5) without 
 * 		allocating a dedicated SAT variable.  For n=8 this removes 65 536 variables and 131 072 clauses.
 *
 *   2. Flat counter arrays. The sequential-counter 2-D table s[i][j] is laid out as a single
 *      contiguous int array of known size (n+1)*(k+1), avoiding the vector-of-vectors overhead and improving cache locality.
 *
 *   3. Fixed-size Walsh literal buffer. The per-ω zero-indicator array is declared on the stack as
 *      int[N_MAX] rather than a heap-allocated vector, since N = 2^n is known at compile time.
 *
 *   4. Optimised forced-cardinality clauses (matches encode.py exactly).
 *      The two reverse-propagation clauses
 *        s[i][j] -> s[i-1][j] ∨ x_i   and   s[i][j] -> s[i-1][j-1]
 *      are only emitted when j <= minimum (l).  For j > l they are not needed to enforce the upper bound and their omission saves
 *      O(n*(k-l)) clauses per counter, which is significant when  k_high ≈ N/2 and l = k_low.
 *
 *   5. O(1) parity via bit-twiddling. ⟨ω,x⟩ = popcount(ω & x) mod 2, computed without a loop.
 *
 *   6. O(1) ANF monomial containment via precomputed bitmasks. S ⊆ supp(x)  iff  (x & mask_S) == mask_S.
 */
...
// Configuration: edit these to change the problem instance
#define N              8    // number of input variables (must be even)
#define MAX_DEGREE     3    // maximum ANF algebraic degree (N = unrestricted)
#define NORMALIZE      1    // 1 = fix f(0) = 0; 0 = allow constant term
...
#define TRACK_TIME     1
#define PRINT_TIME     1
#define MAX_RUNTIME    0    // seconds; <= 0 disables timeout

// Compile-time derived constant: maximum number of inputs.
// Used to size the on-stack Walsh literal buffer.
#define N_MAX          (1 << N)
...
// Clause counters: index = clause length (1,2,3)
static unsigned long long g_clause_count[3] = {0};
...
static void addForcedCardinality(CaDiCaL::Solver& s, int& vc,
	...
	for (int i = 1; i <= n; i++) {
		...
		for (int j = 1; j <= k; j++) {
			...
			// Omitting them saves O(n*(k-l)) clauses, when k = ~N/2 and l = k_low (roughly N/2 - 2^{n/2-1}).
			if (j <= l) {
 ...
 */
static EncoderResult encodeBentFunction(CaDiCaL::Solver& solver) {
	const int n         = N;
	const int max_deg   = MAX_DEGREE;
	...
	enc.anf_vars.push_back({ {}, 0ULL, ++vc });

	// Linear terms (degree 1).
	for (int i = 1; i <= n; i++)
		enc.anf_vars.push_back({ {i}, 1ULL << (i - 1), ++vc });
	...
	for (int deg = 2; deg <= max_deg; deg++) {
		...
		for (auto& sub : combos) {
			...
			enc.anf_vars.push_back({ sub, mask, ++vc });
		}
	}
	...
	//
	// We pre-fill a fixed-size on-stack array zero_lits[N_inputs]
	// (size known at compile time as N_MAX = 2^N) and pass it directly
	// to addAuxCardinality, avoiding a heap allocation per ω.
	//
	// Two auxiliary variables E_low and E_high capture the two exact
	// counts; the clause (E_low ∨ E_high) forces exactly one to hold.
	// ----------------------------------------------------------
	const int k_low  = (1 << (n - 1)) - (1 << (n / 2 - 1));
	const int k_high = (1 << (n - 1)) + (1 << (n / 2 - 1));

	cout << "Bent condition:         #zeros per ω in {" << k_low << ", " << k_high << "}\n";

	// Fixed-size buffer: size is 2^N, known at compile time.
	int zero_lits[N_MAX];
...
int main(int argc, char* argv[]) {
	cout << "Bent Function SAT Encoder / Decoder\n";
	cout << "  n = " << N << ", max_degree = " << MAX_DEGREE << "\n";
	...
	EncoderResult enc = encodeBentFunction(solver);

#if TRACK_TIME == 1
	double enc_wall = chrono::duration<double>(chrono::steady_clock::now() - enc_wall_start).count();
	cout << "Encoding wall time:     " << enc_wall << "s\n";
#endif

    // ---- Output clause counts (before solving) ----
    cout << "\n--- Clause statistics ---\n";
    cout << "  Variables:            " << enc.var_count << "\n";
    ...
    cout << "  Total clauses:        " << (g_clause_count[0] + g_clause_count[1] + g_clause_count[2]) << "\n";
...
#if TRACK_TIME == 1
	g_solve_time = chrono::duration<double>(chrono::steady_clock::now() - solve_start).count();
	cout << "Solve time:             " << g_solve_time << "s\n";
#endif

	double total_elapsed = chrono::duration<double>(chrono::steady_clock::now() - total_start).count();

	// ---- Decode / report ----
	if (result == 10) {         // SAT: a bent function was found
		cout << "Result:                 SAT - bent function found\n";
		decodeSolution(solver, enc);
	} else if (result == 20) {  // UNSAT: no bent function satisfies these constraints
		cout << "\nResult: UNSAT - no bent function satisfies these constraints.\n";
	} else {
		cout << "\nResult: UNKNOWN (solver returned " << result << ")\n";
	}

	cout << "\nTotal elapsed time: " << total_elapsed << "s\n";
	...
	cout << "  Total:                  " << total_elapsed        << "s\n";
#endif

#if MAX_RUNTIME > 0
	if (total_elapsed > MAX_RUNTIME)
		cout << "\n(Timeout: MAX_RUNTIME = " << MAX_RUNTIME << "s)\n";
#endif
